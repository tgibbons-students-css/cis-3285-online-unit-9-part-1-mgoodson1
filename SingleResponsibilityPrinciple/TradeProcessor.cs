...
namespace SingleResponsibilityPrinciple
{
    public class TradeProcessor
    {
        ...
        private bool ValidateTradeData(string[] fields, int currentLine)
        {
            ...
            if (tradeAmount < 1000)
            {
                LogMessage("WARN", " Trade amount on line {0} < 1000: '{1}'", currentLine, fields[1]);
                return false;
            }

            if (tradeAmount > 100000)
            {
                LogMessage("WARN", " Trade amount on line {0} > 100000: '{1}'", currentLine, fields[1]);
                return false;
            }
